...
function renderTag(node, contexts, out) {
  ...
  if (hasChildren || TAGS_NO_SHORT_CLOSING.indexOf(tagName) !== -1) {
    out.push('>');
    if (hasChildren) {
      tagAttrs.children.forEach(function(child)  {return _renderToString(child, out, contexts);});
    }
    out.push('</'+tagName+'>');
  } else {
    out.push(' />');
...
var _renderToString = function(node, out, contexts) {
  ...
  if (Array.isArray(node)) {
    node.forEach(function(n)  {return _renderToString(n, out, contexts);});
    return;
  }
  ...
  if (isFunction(node.type)) {
    ...
    var args = [props];
    contexts.forEach(function(c)  {return args.push(c);});
...
var renderToString = function(node ) {for (var contexts=[],$__0=1,$__1=arguments.length;$__0<$__1;$__0++) contexts.push(arguments[$__0]);
  var out = [];
